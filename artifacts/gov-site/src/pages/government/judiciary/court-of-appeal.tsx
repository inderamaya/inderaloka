    ...
    <div className="flex-1 w-full bg-background pb-24">
      <div className="bg-secondary border-b border-border py-12">
        <div className="container mx-auto px-4 lg:px-8">
          ...
          <h1 className="font-serif text-4xl lg:text-5xl text-primary mb-6">{ca.pageTitle}</h1>
          <p className="text-muted-foreground text-lg max-w-2xl font-light">{ca.pageDesc}</p>
        </div>
      </div>

      <div className="container mx-auto px-4 lg:px-8 pt-16 flex flex-col gap-16">
        <section>
          <h2 className="font-serif text-2xl text-primary mb-4">{ca.overviewTitle}</h2>
          <div className="w-12 h-px bg-accent mb-6" />
          <p className="text-muted-foreground leading-relaxed font-light max-w-3xl">{ca.overviewBody}</p>
        </section>

        <section>
          <h2 className="font-mono text-[10px] uppercase tracking-widest text-accent mb-6">{ca.presidentLabel}</h2>
          <div className="border border-border bg-secondary p-6 inline-block">
            <p className="font-serif text-xl text-primary">{ca.presidentName}</p>
          </div>
        </section>

        <section>
          <h2 className="font-serif text-2xl text-primary mb-4">{ca.jurisdictionTitle}</h2>
          <div className="w-12 h-px bg-accent mb-6" />
