...
import { ArrowRight, ChevronRight, Landmark, Gavel, FileText } from "lucide-react";

export default function Legislative() {
  const { t } = useLanguage();
  const g = t.government;
  const l = g.legislative;
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <Breadcrumb className="mb-8 font-mono uppercase tracking-[0.3em] text-[10px] font-bold text-accent">
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator className="opacity-50" />
              <BreadcrumbItem><BreadcrumbPage className="text-primary">{l.breadcrumbCurrent}</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
          <h1 className="font-serif text-5xl lg:text-7xl font-bold text-primary mb-8 leading-tight tracking-tighter">{l.pageTitle}</h1>
          <p className="text-muted-foreground text-xl font-light max-w-2xl leading-relaxed">{l.pageDesc}</p>
        </div>
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {l.subpages.map((sub, idx) => (
            ...
            >
              <Link href={sub.href}>
                <GlassCard className="h-full p-10 cursor-pointer border-transparent hover:border-accent/30 transition-all duration-500 group bg-white/40 dark:bg-black/20">
                  ...
                  <div className="flex items-center gap-2 text-[10px] font-mono font-bold uppercase tracking-widest text-primary opacity-0 group-hover:opacity-100 transition-all">
                    View Chamber <ChevronRight className="w-3 h-3" />
                  </div>
                </GlassCard>
              </Link>
            </motion.div>
