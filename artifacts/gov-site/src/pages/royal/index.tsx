    ...
    <div className="flex-1 w-full bg-background pb-24">
      <div className="bg-secondary border-b border-border py-12">
        <div className="container mx-auto px-4 lg:px-8">
          ...
          <p className="text-muted-foreground text-base sm:text-lg max-w-2xl font-light leading-relaxed">{l.pageDesc}</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-px bg-border border border-border">
          ...
          {l.subpages.map((sub, idx) => (
            <motion.div key={sub.href} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: idx * 0.08 }}>
              <Link href={sub.href} className="group bg-background p-8 flex flex-col h-full hover:bg-primary hover:text-primary-foreground transition-colors" data-testid={`card-royal-${idx}`}>
